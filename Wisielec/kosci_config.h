...
enum class TrybGry { LOKALNY, SOLO_BOT, HOST, KLIENT };
enum class TypGracza { CZLOWIEK, SIECIOWY, BOT };

enum class Kategoria {
    Jedynki, Dwojki, Trojki, Czworki, Piatki, Szostki,
    Trojka, Czworka, Full, MalyStrit, DuzyStrit, Yahtzee, Szansa
};
...
struct StanGracza {
    ...
    int sumaGor() const {
        int s = 0;
        std::vector<Kategoria> k = { Kategoria::Jedynki, Kategoria::Dwojki, Kategoria::Trojki,
                                    Kategoria::Czworki, Kategoria::Piatki, Kategoria::Szostki };
        for (auto cat : k) if (zajete.value(cat)) s += wynik.value(cat);
        return s;
    }
    int bonus() const { return sumaGor() >= 63 ? 35 : 0; }
    int sumaDol() const {
        int s = 0;
        for (auto k : wynik.keys()) s += wynik[k];
        return s - sumaGor();
    }
    int total() const { return sumaGor() + bonus() + sumaDol(); }
    bool koniec() const { return zajete.size() >= 13; }
};
